...
void Plate::channeTimeConstraint(){
	...
	for(Channel c:channels){
		...
		Device& childDevice = c.childOp.bindDevice;
...
void Plate::channelPlaceConstraint(){
	...
	for(Channel c:channels){
		...
		Device d0 = getDeviceFromOp(c.fatherOp);
		Device d1 = getDeviceFromOp(c.childOp);
	...
	for(Channel c:channels){
		...
		for(int i = 2; i<=3*(c.vertexNum -1) ; i++){
			...
			for(int j = 1; j <= i-1;j++){
				...
				string up = seg1Name +  S("Up") + c.name + seg0Name;
				varName.push_back(up); varType.push_back("1");
				string down = seg1Name +  S("Down") + c.name + seg0Name;
				varName.push_back(down); varType.push_back("1");
				string left = seg1Name +  S("Left") + c.name + seg0Name;
				varName.push_back(left); varType.push_back("1");
				string right = seg1Name +  S("Right") + c.name + seg0Name;
				varName.push_back(right); varType.push_back("1");
...
void Plate::setChannelsTime()
{
	...
	for(Op child:seq.getOps())
	{
		Device& childDevice = child.bindDevice;
		for(Op parent:child.parents){
			Channel c;
			Device& fatherDevice = child.bindDevice;
